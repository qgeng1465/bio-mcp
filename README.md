...
>
> BioMCP 是一个开源的 **MCP 服务器**，让任意 AI 助手**零配置直连 15 个公开生物数据库**——文献、序列、比对、结构、富集、注释、基因组、互作、变异、结构域、化合物、单细胞等全流程。

![MCP](https://img.shields.io/badge/MCP-Server-7c5cff?style=flat-square)
![Tools](https://img.shields.io/badge/Tools-23-0ea5e9?style=flat-square)
![Databases](https://img.shields.io/badge/Databases-15-22c55e?style=flat-square)
...
- 🧬 **23 个工具 · 15 个数据库** / 23 tools · 15 databases — 覆盖文献→序列→结构→功能→互作→变异→药物→单细胞全流程
- ⚡ **零配置使用** / Zero-config — `pip install bio-mcp` 一条命令，无需数据库、无需密钥
- 🌐 **数据公开权威** / Authoritative public data — 全部来自 NCBI / RCSB / UniProt / Ensembl / EBI / STRING / KEGG 等官方 API
...
在任何支持 MCP 的 AI 助手 / IDE 中注册 BioMCP（Claude Code / Cursor / VS Code 等）：
...
OpenGWAS 从 2024-05 起强制要求 API token，无法零配置直连，故不包含。所有收录的 15 个数据库均为开放免密钥 API。
...
bio-mcp/
├── src/bio_mcp/
│   ├── server.py            # MCP server 入口（装配 23 个工具）
│   ├── core/                # 15 个数据源客户端
...
> 全部 23 个工具均已在开发期对真实公开数据库做端到端验证（23/23 通过）。
> All 23 tools were end-to-end validated against the real public databases (23/23 passed).

---

## 🗺️ Roadmap / 路线图

- 微生物 / 病毒 / 细菌数据库（NCBI Assembly、Virus、病原基因组）
- 基因组 / 核酸结构数据库（ENA、BioSample、质粒数据库）
- 糖组学 / 脂质组学数据库
- 批量对比分析（多条序列/多基因批量富集）
